def ask_question(question):
    while True:
        ...
        if response == 'A' or response == 'B':
            ...
            print(" Invalid attempt, Please enter either 'A' or 'B'. Try again.")


def extrovert_or_introvert(extrovert_count, introvert_count):
    return "I" if extrovert_count < introvert_count else "E"


def sensitive_or_intuitive(sensitive_count, intuitive_count):
    return "I" if sensitive_count < intuitive_count else "S"


def thinker_or_feeler(thinker_count, feeler_count):
    return "F" if thinker_count < feeler_count else "T"


def judging_or_perspective(judging_count, perspective_count):
    return "P" if judging_count < perspective_count else "J"
...
questions = [
    ...
    "9) A. Seek many tasks, public activities, interaction with others. B. Seek private, solitary activities with quiet"
    "to concentrate.",
...
personality_descriptions = {" ENFJ ":
                                """A ENTJ (ENFJ) is a person with the Extraverted, Intuitive,
Feeling, and Judging personality traits. These warm, forthright types love helping others, and they tend to have strong 
ideas and values. They back their perspective with the creative energy to achieve their goals.
...
user_responses = [0] * len(questions)

print("What's your name: ")
user_name = input()

for i in range(len(questions)):
    user_responses[i] = 1 if ask_question(questions[i]) == "A" else 2

personality_type = (
        extrovert_or_introvert(user_responses[0], user_responses[1])
        + sensitive_or_intuitive(user_responses[2], user_responses[3])
        + thinker_or_feeler(user_responses[4], user_responses[5])
        + judging_or_perspective(user_responses[6], user_responses[7])
)

print(personality_type)

print(f"\n{user_name}, your personality type is: {personality_type}")
if personality_type in personality_descriptions:
    print(personality_descriptions[personality_type])
else:
    print("Personality type description not available yet. or check entered response")
